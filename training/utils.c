...
int connect_to_device(cl_device_type dev_type, cl_platform_id *platform, cl_device_id *device_id)
{
    ...
    if (num_plat == 1) {
        printf("There is 1 platform available\n");
    } else {
        printf("There are %u platforms available\n", num_plat);
    }
    ...
    for (unsigned int i = 0; i < num_plat; i++) {
        ...
        if (err_code != CL_SUCCESS || num_devs <= 0) {
            continue;
        }
...
int ***read_histogram_file(const char *filename)
{
	FILE *f = NULL;
	...
	int ***frequency = calloc(3, sizeof(int**));
	if(NULL == frequency) {
	...
	for(int i = 0; i < 3; i++){
		frequency[i] = calloc(3, sizeof(int*));
		if(NULL == frequency[i]) {
			printf("Cannot allocate memory!");
			exit(-1);
		}
		for(int j = 0; j < 3; j++){
			frequency[i][j] = calloc(256, sizeof(int));
			if(NULL == frequency[i][j]) {
				printf("Cannot allocate memory!");
				exit(-1);
			}
		}
	...
	while(fscanf(f, "%d", &value) == 1) {
		...
		int engininami = ((int) x / 30);
		int ahmetinami = ((int) y / 30);
		
		frequency[ahmetinami][engininami][value]++;
		x++;
	}
	fclose(f);
	return frequency;
...
const char *get_ocl_error(cl_int error)
{
    switch(error) {
        ...
        case(CL_MEM_COPY_OVERLAP):
            return "Overlap detected in memory copy operation!";

        case(CL_IMAGE_FORMAT_MISMATCH):
            return "Image format mismatch detected!";
        ...
        case(CL_INVALID_HOST_PTR):
            return "Invalid host pointer address!";
        ...
        case(CL_INVALID_ARG_INDEX):
            return "Invalid index for kernel argument!";

        case(CL_INVALID_ARG_VALUE):
            return "Invalid value for kernel argument!";

        case(CL_INVALID_ARG_SIZE):
            return "Invalid size for kernel argument!";
